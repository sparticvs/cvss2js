...
function getScore(input, options = {}) {
    ...
    if(typeof input === "string") {
        ...
        // TODO sanitization function
        vector = input;
    } else {
        throw new Error("Invalid type for 'input'");
    }

    return __computeEnvironmentalScore(vector);
}
...
function getBaseScore(input, options = {}) {
    ...
    const vector = __parseVectorString(input);
    return __computeBaseScore(vector);
}
...
function getTemporalScore(input, options = {}) {
    ...
    const vector = __parseVectorString(input);
    return __computeTemporalScore(vector);
}
